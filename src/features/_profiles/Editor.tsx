...
const Editor = () => {
    ...
    const [user, setUser] = useState({user_name:'', email:'', link_avatar:''})
    const [uploadedImage, setUploadedlImage] = useState<File | null>(null);
    useEffect(()=>{
        fetch(`https://metatechvn.store/profile/${id}`,{
            method:"GET",
            headers: {
        ...
        <div className="">
                    ...
                    <div className="flex my-auto mt-[45px]">
                        <input
                            ...
                            />
                        <div className="w-[96px] h-[96px] rounded-[48px] overflow-hidden" {...getRootProps()}>
                            {uploadedImage ? (
                                    <img className="aspect-square w-full" src={URL.createObjectURL(uploadedImage)} />
                                    ) : (
                                    <img src={`${user.link_avatar}`} className="h-full" alt="" />)
                                    }
                        </div>
                        <button onClick={open} className="text-[#fff] w-[253px] bg-[#16B6D4] h-[54px] my-auto ml-8 rounded-[27px] px-[20px] py-[10px] text-center font-[700] text-[16px] leading-[24px] flex mx-auto items-center">
